...
class GraphTransformerClassifier(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for layer in self.transformer:
            tokens, _ = layer(tokens, graph_bias)  # Discard attention weights during forward
...
class GraphAwareMultiheadAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_bias: torch.Tensor = None) -> tuple:
        B, L, D = x.shape
        ...
        # Return output and attention weights (before dropout) for interpretability
...
class GraphTransformerLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_bias: torch.Tensor = None) -> tuple:
        residual = x
        x = self.norm1(x)
        attn_output, attn_weights = self.self_attn(x, attn_bias=attn_bias)
