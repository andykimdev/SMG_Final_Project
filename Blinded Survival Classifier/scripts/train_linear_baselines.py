"""
Linear baseline models for BLINDED survival prediction.
Tests if simple linear methods (PCA + Cox, Elastic Net Cox) can match neural networks.

Uses ONLY blinded features (protein + Age/Sex/Race/Ancestry, NO genomics).
"""
...
from blinded_survival_classifier.data import load_graph_prior, load_and_preprocess_survival_data
...
def prepare_survival_data(data_splits, survival_info):
    ...
    CRITICAL: Concatenates ALL features (protein + clinical + genomic)
    For blinded model: genomic will be empty, so just protein + clinical
    """
    print(f"\nPreparing survival data for linear baselines...")

    def extract_data(split_tuple):
        """
        Extract and concatenate ALL features from split tuple.

        Expected tuple: (X_protein, X_clinical, X_genomic, event, time)
        Returns: (X_all_concatenated, y_df)
        """

        if len(split_tuple) == 5:
            X_protein, X_clinical, X_genomic, event, time = split_tuple

            print(f"  Protein features: {X_protein.shape}")
            print(f"  Clinical features: {X_clinical.shape}")
            print(f"  Genomic features: {X_genomic.shape}")

            # Concatenate ALL feature types (genomic will be empty for blinded)
            feature_arrays = [X_protein]
            if X_clinical.shape[1] > 0:
                feature_arrays.append(X_clinical)
            if X_genomic.shape[1] > 0:
                feature_arrays.append(X_genomic)

            X = np.concatenate(feature_arrays, axis=1)
            print(f"  Total concatenated features: {X.shape}")
...
def main():
    ...
    print("\n⚠️  BLINDED MODEL: Using only IHC-available features")
    print("   ✅ Protein expression, Age, Sex, Race, Ancestry")
    print("   ❌ Cancer type, tumor stage, genomic features\n")
