"""
BLINDED TCGA RPPA dataset module for Disease-Specific Survival (DSS) prediction.
Uses ONLY features available at initial IHC pathology visit (pre-genomic testing).

INCLUDED FEATURES:
- Protein expression (RPPA/IHC)
- Age
- Sex
- Race
- Genetic ancestry

EXCLUDED FEATURES (not available at IHC visit):
- Cancer type/subtype information
- Tumor stage
- Cancer status
- Sample type
- Genomic features (TMB, MSI, aneuploidy, etc.)
"""
...
def identify_column_types(df: pd.DataFrame) -> Dict[str, List[str]]:
    """
    Identify column types for BLINDED survival prediction.
    Only includes features available at initial IHC pathology visit.

    ALLOWED: Protein expression, Age, Sex, Race, Genetic ancestry
    EXCLUDED: Cancer type, tumor stage, genomic features, cancer status
    """
    columns = {
        'identifiers': [],
        'protein': [],
    ...
    # BLINDED: Exclude ALL cancer-specific and genomic information
    excluded_keywords = [
        'CANCER_TYPE_ACRONYM', 'ONCOTREE_CODE', 'TUMOR_TYPE', 'SUBTYPE',  # Cancer type
        'AJCC_PATHOLOGIC_TUMOR_STAGE', 'SAMPLE_TYPE',  # Tumor staging
        'PERSON_NEOPLASM_CANCER_STATUS',  # Cancer status (post-diagnosis)
    ]
    columns['target_exclusions'] = [col for col in df.columns if col in excluded_keywords]

    # BLINDED: NO genomic features (not available at IHC visit)
    genomic_keywords = ['ANEUPLOIDY', 'TMB', 'MSI', 'TBL']
    excluded_genomic = [col for col in df.columns
                       if any(kw in col for kw in genomic_keywords)]
    columns['target_exclusions'].extend(excluded_genomic)
    columns['genomic'] = []  # Always empty for blinded classifier

    # BLINDED: Only basic demographic/patient info available at IHC visit
    ...
    # BLINDED: Do NOT include cancer type - not available at IHC visit
    # Cancer type is explicitly excluded for blinded prediction
    
    print("\nBLINDED Column Identification (IHC-available features only):")
    print(f"  Identifiers: {len(columns['identifiers'])}")
    print(f"  Proteins (IHC): {len(columns['protein'])}")
    ...
    print(f"  Genomic: {len(columns['genomic'])} (ALWAYS 0 for blinded model)")
    print(f"  Survival (targets): {len(columns['survival'])}")
    print(f"  Excluded (cancer type, stage, genomic): {len(columns['target_exclusions'])}")
