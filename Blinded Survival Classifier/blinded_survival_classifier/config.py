"""
Configuration file for BLINDED Graph Transformer Survival Predictor
...
MODEL = {
    'embedding_dim': 256,       # Dimension of token embeddings (v3_wider: optimal size)
    'n_layers': 4,              # Number of transformer layers (v3_wider: optimal depth)
    'n_heads': 8,               # Number of attention heads (optimal)
    'ffn_dim': 640,             # Feedforward network dimension (4x embedding_dim)
    'dropout': 0.4,            # Dropout probability (optimal regularization)
    'pe_dim': 16,               # Graph positional encoding dimension (k eigenvectors)
    'graph_bias_scale': 0.25,   # Initial scale for graph-aware attention bias (optimal balance)
}
