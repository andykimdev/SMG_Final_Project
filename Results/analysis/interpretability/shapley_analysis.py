...
def compute_shap_values(model, background_data, test_data, device='cpu', 
                       n_background=40, n_test=100):
    """
    ...
    Args:
        ...
        device: torch device
        n_background: Number of background samples (default 40)
        n_test: Number of test samples to explain (default 100)
    ...
    bg_subset = background_data[:min(n_background, len(background_data))]
    test_subset = test_data[:min(n_test, len(test_data))]
    
    print(f"Using {len(bg_subset)} background samples and {len(test_subset)} test samples")
...
def main():
    ...
    train_x = data_splits['train'][0]
    test_x = data_splits['test'][0]
    
    print(f"Background set: {len(train_x)} samples")
